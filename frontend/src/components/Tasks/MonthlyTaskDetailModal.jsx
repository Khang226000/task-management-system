/**
 * MonthlyTaskDetailModal — Modal chi tiết + chỉnh sửa CV tháng
 * Dùng chung cho: StatsMonthlyPage, UnifiedCalendarPage, TeamTasksPage
 */
...
export default function MonthlyTaskDetailModal({ task: initialTask, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    startDate:      cd(initialTask.startDate),
    dueDate:        cd(initialTask.dueDate),
    extendedDueDate:cd(initialTask.extendedDueDate),
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <div>
              ...
              <select className="select" value={form.department} onChange={e => s('department', e.target.value)}>
                ...
                {['KN&DMST', 'Hành chính tổng hợp', 'Thông tin thống kê', 'Dịch vụ'].map(d => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
          </div>
